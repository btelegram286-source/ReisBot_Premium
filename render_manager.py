...
class RenderManager:
    ...
    def create_service(self, service_name, github_repo_url, branch="main"):
        """Yeni Render servisi oluştur"""
        try:
            # Render API'si için basit payload
            payload = {
                "name": service_name,
                "type": "web_service",
            ...
            if response.status_code == 201:
                service = response.json()
                return f"✅ Yeni servis oluşturuldu: {service_name}"
            else:
                error_msg = response.text
                if response.status_code == 409:
                    return f"⚠️ Servis zaten mevcut: {service_name}"
                return f"❌ Servis oluşturma hatası ({response.status_code}): {error_msg}"
